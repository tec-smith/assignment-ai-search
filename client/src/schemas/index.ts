...
export const BookingInputSchema = z.string().refine((input) => {
  ...
  const pattern = /^([^,]+),\s*([^,]+)(?:\s+(\d{1,2}(?:st|nd|rd|th)\s+\w+\s+to\s+\d{1,2}(?:st|nd|rd|th)\s+\w+))?\s+for\s+(\d+)\s+person(s?)$/i;
  ...
  message: 'Invalid input format. Expected format: "Location, Country [from date to date] for [number] person(s)"',
});
